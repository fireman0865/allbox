...
class DeviceConfigsView(ObjectView):
    queryset = Device.objects.all()
    template_name = 'dcim/configs.html'
    
    '''
    def get(self, request, pk):
       
        configurations = Config.objects.filter(device=pk)
        

        config_table = tables.DeviceConfigTable(
            configurations)
        

        return render (request ,'dcim/configs.html',{
    ...
    def get(self, request, pk):
        ...
        device = get_object_or_404(self.queryset, pk=pk)
        configs = Config.objects.filter(device=pk)
        

        config_table = tables.DeviceConfigTable(configs)
...
class ConfigListView(generic.ObjectListView):
    ...
    filterset = filters.InterfaceFilterSet
    filterset_form = forms.InterfaceFilterForm
    table = tables.devices.ConfigTable(data=queryset)
    action_buttons = ('import', 'export')
    template_name = 'dcim/config_list.html'
...
def ConfigView(request,**kwargs):
    pk=kwargs['pk']
    files=Config.objects.filter(pk=pk).values()
    files=files[0]['files']
    files='/home/salman/Pictures/box/netbox/netbox/media/configs/'+str(files)
    response = HttpResponse(open(files, 'r').read())
    response['Content-Type'] = 'text/plain'
    response['Content-Disposition'] = "attachment; filename='{}'".format(files)
    return response

class ConfigListView(ObjectListView):
    queryset = Config.objects.all()
    table = tables.ConfigTable
    #template_name = 'dcim/device_list.html'
    filterset = filters.ConfigFilterSet
    filterset_form = forms.ConfigFilterForm
    action_buttons = None



import difflib
class ConfigDiffView(ObjectView):
    queryset = Device.objects.all()
    template_name='dcim/configdiff.html'
    form = forms.NameForm1()
    status_code=200
    ...
    def post(self, request,*args,**kwargs):
        ...
        form = forms.NameForm1(request.POST)

    

        config1=Config.objects.filter(config_date__contains=date1).values()
        config2=Config.objects.filter(config_date__contains=date2).values()

        if form.is_valid() :

            if len(config1)!=0 and config2 is not None:
                config1=config1[0]['files']
                file1='/home/salman/Pictures/box/netbox/netbox/media/configs/'+config1
                config2=config2[0]['files']
                file2='/home/salman/Pictures/box/netbox/netbox/media/configs/'+config2
        

                file_config =open(file1,'r')
                file_config1 =open(file2,'r')
            

                diff = difflib.HtmlDiff().make_file(fromlines=file_config1.readlines(),tolines=file_config.readlines())
             # ...
            # redirect to a new URL:
                status=200
                return render (request, self.template_name, {'diff':diff})
            else:
                messages.error(request, "Date {} and {} don't correspond.Please choose some others dates".format(date1,date2))
